...
import { softDelete } from '@/app/(protected)/receipt/actions/receipt';
...
const RecTable = () => {
  const { isOpen , isHover ,darkMode , pending,setPending,onEdit,setPassingId , setRec} = useToggle()
  ...
  const {data , error} = useSWR(`${url}/api/recTable?email=${user.id}&name=${user.name}&page=${page}&take=${take}&filter=${val.filter}&filter1=${val.filter1}&fromDate=${val.fromDate}&toDate=${val.toDate}`,fetchData)
  ...
  useEffect(()=>{
    if(!val.filter || !val.filter1){
      setPage(currentPage)
      mutate(`${url}/api/recTable?email=${user.id}&name=${user.name}&page=${currentPage}&take=${take}&filter=${val.filter}&filter1=${val.filter1}&fromDate=${val.fromDate}&toDate=${val.toDate}`)
    }
    if(val.filter !== '' || val.filter1 !== ''){
      setPage(1)
      mutate(`${url}/api/recTable?email=${user.id}&name=${user.name}&page=1&take=${take}&filter=${val.filter}&filter1=${val.filter1}&fromDate=${val.fromDate}&toDate=${val.toDate}`)
    }
  ...
  ? rec.filter(rec => areAnagrams(rec.recBus, anagramFilter))
  : rec;
  ...
  return (
   <>
    ...
    <div className='flex py-3 justify-between'>
        ...
        {/****************** */}
        <Modal typeSelect='caution' id='receipts' handlingAction={()=>handleDelete(passing)} CautionText={'Deletion'}/>
    </div>
