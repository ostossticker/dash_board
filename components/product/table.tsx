...
import { useProduct } from '@/hooks/usedatas';
...
const ProdTable = () => {
  ...
  const {data , error} = useSWR(`${url}/api/producttable?email=${user.id}&name=${user.name}&page=${page}&take=${take}&filter=${val.filter}&filter1=${val.filter1}`,fetchData)
  ...
  useEffect(()=>{
    if(!val.filter || !val.filter1){
      setPage(currentPage)
      mutate(`${url}/api/producttable?email=${user.id}&name=${user.name}&page=${currentPage}&take=${take}&filter=${val.filter}&filter1=${val.filter1}`)
    }
    if(val.filter !== '' || val.filter1 !== ''){
      setPage(1)
      mutate(`${url}/api/producttable?email=${user.id}&name=${user.name}&page=1&take=${take}&filter=${val.filter}&filter1=${val.filter1}`)
    }
    if(val.filter1 !== ""){
  ...
  return (
    <div className={`${darkMode ? "bg-dark-box-color" : "bg-white"} mt-[20px] shadow-md rounded-lg px-[30px] ${isOpen || isHover ? "py-[8px]" : "py-[10px]"}`}>
                            ...
                            {test.map((item)=>{
                              return(
                                <li data-id={item.id} className={`cursor-pointer ${selectedItemId === item.id ? "bg-gray-200" : "bg-transparent" }`} key={item.id} onClick={()=>{
                                  handleClick('filter1' , item.busName)
                                }}>
                                  {item.busName}
                                </li>
                              )
              ...
              <tr>
                {
                  thead?.map((item,i)=>{
                    return(
                      <th key={item.label}  className={`${item.textAlign} ${i === 0 ? 'rounded-tl-md' : ''} ${i === 5 ? 'rounded-tr-md !pr-[50px]' : ''} xl:text-[16px] lg:text-[10px] xl:leading-7 pt-[3px] text-white bg-thead-primary text-[15px]`}>{item.label}</th>
                    )
                  })
                }
              </tr>
            ...
            <tbody>
              {
                filteredUsers?.map((item,i)=>{
                  ...
                  <tr key={item.id} className={`${darkMode ? "bg-dark-box-color text-dark-lg-color" : "bg-white"} xl:text-[16px] lg:text-[11px] hover:bg-[#F9FAFB]`}>
                    ...
                    <td className={`${placeholderClass} text-start`}>{item.prodItemName}</td>
                    <td className={`${placeholderClass} text-start`}>{item.proditemDes}</td>
                    <td className={`${placeholderClass} text-start`}>{item.prodBus}</td>
                    <td className={`${placeholderClass} text-end`}>${parseFloat(item.prodUnitPrice).toFixed(2)}</td>
        ...
        {/****************** */}
        <Modal typeSelect='caution' id='products' handlingAction={()=>handleDelete(passing)} CautionText={'Deletion'}/>
    </div>
  )
}
